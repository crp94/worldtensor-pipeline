"""Download MCD64A1 burned area monthly granules via earthaccess.

Usage:
    python -m src.download.mcd64a1 --all
    python -m src.download.mcd64a1 --start-year 2015 --end-year 2020
"""
...
logger = get_logger("download.mcd64a1")
...
def _import_earthaccess():
    try:
        ...
        raise RuntimeError(
            "earthaccess is required for MCD64A1 download. "
...
def search_granules(start_year: int | None = None, end_year: int | None = None) -> list:
    """Search MCD64A1 monthly granules in CMR."""
    earthaccess = _import_earthaccess()
    ...
    logger.info("Found %d MCD64A1 granules", len(granules))
    return granules


def download_mcd64a1(
    start_year: int | None = None,
    end_year: int | None = None,
    raw_dir: Path = DEFAULT_RAW_DIR,
    threads: int = 8,
) -> list[Path]:
    """Authenticate, search, and download MCD64A1 granules.
    ...
    # Use earthaccess to download with verification
    downloaded = earthaccess.download(granules, str(raw_dir), threads=threads)
    paths = []
...
def download_granules_direct(
    granules: list,
    raw_dir: Path = DEFAULT_RAW_DIR,
    threads: int = 16,
) -> list[Path]:
    """Download pre-searched granules without re-authenticating or re-searching.

    Use with search_granules() to avoid repeated auth + CMR queries.
    """
    earthaccess = _import_earthaccess()
    if not granules:
        return []
...
def main(run_all, start_year, end_year, raw_dir):
    """Download MCD64A1 monthly granules via earthaccess."""
    ...
    paths = download_mcd64a1(start_year=start_year, end_year=end_year, raw_dir=out_dir)
